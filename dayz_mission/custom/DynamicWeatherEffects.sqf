...
// Fog intensity never exceeds this value. Must be between 0 and 1 and greater than or equal to than _minimumFog
...
// Wind vector strength in never falls below this value. Must be greater or equal to 0 and less than or equal to _maximumWind.
// (Suggested value: 0);
_minimumWind = 0;
...
// goes above 0.75, script a chain of rain intervals (defined below) is started. It cycles on until overcast falls below 0.75. At overcast 
...
_rainIntervalRainProbability = 1;
...
// If set to true, then the rain is forced to stop after one rain interval during which it has rained (use this if you for exampel only 
// want to have small occational cloudbursts). If set to false, then the rain may stop, but it may also just change intensity for an 
